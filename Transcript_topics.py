...
y1 = df_year.loc[:,[y for y in range(1982,1992)]].mean(axis=1).rolling(window_size).mean()
y2 = df_year.loc[:,[y for y in range(1993,2003)]].mean(axis=1).rolling(window_size).mean()
y3 = df_year.loc[:,[y for y in range(2004,2013)]].mean(axis=1).rolling(window_size).mean()

df_joint = pd.DataFrame([y1,y2,y3],index = ['1982-1992','1993-2003','2004-2013']).T
...
def saveFile(fname,year,text):
    main_directory = '/Users/LENOVO USER/Desktop/FedTranscript1/cropped/'
    ...
    if not os.path.isfile(fname):#check if file name already exists
        os.chdir(directory)
        file= open(fname, 'w', encoding="ascii", errors = 'ignore')
        file.write(text)
        file.close()
                
doc_start= {}
doc_start[0] = "Transcript  of  Federal  Open  Market  Committee  Meeting"
#doc_start[1] = re.compile('The information (reviewed|received|provided)')
doc_start[2] = "Federal Open Market Committee Meeting"
doc_start[3] = "Meeting"
doc_start[3] = "Federal  Open Market  Committee"
...
print(TreebankWordDetokenizer().detokenize(sent_example[0]))

print('\n'+'\033[1m'+"Tokenize"+'\033[0m')
print(sent_example[0])

print('\n'+'\033[1m'+"Lowecase & remove punctuation/integers"+'\033[0m')
...
'go', 'mr', 'thank', 'month', 'ms', 'mr ', 'ms ', 'appendix',  ]
stopwords.extend(newStopWords)


remove_items = stopwords + list(string.punctuation)
sent_clean = [i for i in map(str.lower,sent_example[0]) if i not in remove_items and not i.isdigit()]
#print(sent_clean)
...
sent_stemmed = stem_text(sent_example[0],stem,remove_items)
print(sent_stemmed)
...
sent_lemma = lemmatize_text(sent_example[0],lemma,remove_items)
print(sent_lemma)
...
#%%
vec = CountVectorizer(tokenizer = lambda x: x,lowercase = False, encoding="ascii", decode_error='ignore', ngram_range=(2, 2), token_pattern='/[A-Za-z0-9_]+(?=\s+)/')
#'[a-zA-Z0-9]{4,}'  decode_error='ignore', strip_accents='ascii', 
...
lda = LatentDirichletAllocation(n_components = num_topics,
                                ...
                                random_state =1, learning_decay =0.9).fit(vec_fitted)
...
df_results = summarize_topics(lda,vec.get_feature_names(),20)
transcript_topics = df_results
transcript_lda = lda_t
...
data = dict(data=[trace],layout= {'title':'Top 100 Words per Topic, Transcripts ,ngram=2, 7 topics','width':1400,'height':2000})
py.offline.iplot(data, filename = 'pandas_table')
py.io.write_html(data, file='Transcripts_TopicWords.html', auto_open=True)

#%%
#Dictionary with topivs
topic_dict = {0 : 'Economic Outlook',
              1 : 'Monetary Policy',
              2 : 'Inflation', 
              3 : 'Labor/Employment', 
              4 : 'Recession/QE',
              5 : 'Financial Markets', 
              6 : 'Economic Recovery/growth',
           #   7 : 'Policy',
        #      8 : '8',
         #     9 : '9',            
              }
...
df_topic_m.to_csv("/Users/LENOVO USER/Desktop/Transcripts_Proportion.csv", sep=",")
...
layout = go.Layout(yaxis = dict(ticksuffix='%',showgrid = False),
                   xaxis = dict(type ='category',showgrid = False,
                                ...
                                ticktext = ticktext,showticklabels=True),
                                   title = 'Topic Proportions For Transcripts, 7 Topics, Trained on Transcripts, ngram = 2, learning_decay =0.9'
                  ) 

fig = go.Figure(data=dataProportion,layout=layout)
py.offline.iplot(fig, filename='stacked-area-plot-norm', validate=False)
py.io.write_html(fig, file='Transcripts_Topics_Proportions.html', auto_open=True)
...
py.io.write_html(fig, file='Transcripts_Topics_Sentiments.html', auto_open=True)
df_sentiment_m.to_csv("/Users/LENOVO USER/Desktop/Transcripts_Topics_Sentiment.csv", sep=",")
